...
class ConfigArrayFactory {
    ...
    *_normalizeObjectConfigDataBody(
    ...
    _loadExtendedShareableConfig(extendName, importerPath, importerName) {
        const { cwd } = internalSlotsMap.get(this);
        const relativeTo = importerPath || path.join(cwd, ".eslintrc");
        let request;
    ...
    _loadPlugin(nameOrPath, importerPath, importerName) {
        ...
        try {
            ...
            const relativeTo = path.join(cwd, ".eslintrc");
            const filePath = ModuleResolver.resolve(request, relativeTo);
            ...
            debug("Failed to load plugin '%s' declared in '%s'.", nameOrPath, importerName);
            ...
            error.message = `Failed to load plugin '${nameOrPath}' declared in '${importerName}': ${error.message}`;
